...
export const handlers = [
  ...
  http.post('/api/v1/blends', async ({ request }) => {
    try {
      ...
      const existingIds = [...mockBlends(), ...createdBlends].map(
        (blend) => blend.id,
      );
      const newId =
        existingIds.length > 0 ? Math.max(...existingIds) + 1 : 1000;
  ...
  http.get('/api/v1/blends/:id', ({ params }) => {
    ...
    const blend = [...mockBlends(), ...createdBlends].find(
      (blend) => blend.id === Number(params.id),
    );
